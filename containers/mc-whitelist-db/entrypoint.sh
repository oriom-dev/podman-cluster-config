...
wait_for_sql() {
  attempts=0
  until cockroach sql --certs-dir="$CERTS_DIR" --host="$LISTEN_ADDR" -e "SELECT 1" >/dev/null 2>&1; do
    attempts=$((attempts + 1))
    if [ "$attempts" -ge 60 ]; then
      echo "Timed out waiting for Cockroach SQL listener at $LISTEN_ADDR" >&2
      exit 1
    fi
    sleep 1
  done
...
bootstrap_app_user() {
  ...
  cockroach sql --certs-dir="$CERTS_DIR" --host="$LISTEN_ADDR" -e "
...
if [ ! -f "$BOOTSTRAP_MARKER" ]; then
  ...
  wait_for_sql
  bootstrap_app_user
  cockroach quit --certs-dir="$CERTS_DIR" --host="$LISTEN_ADDR" || true
